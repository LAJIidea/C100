...
namespace C100
{
    ...
    public:
        ...
        static std::shared_ptr<BuildInType> UShorTy;
    ...
    public:
        ...
        FunctionType(std::shared_ptr<Type> returnTy): Type(TypeClass::Function, 1, 1), ReturnType(returnTy) {
            Complete = false;
        }
    };
    ...
    public:
        ...
        PointerType(std::shared_ptr<Type> baseTy): Type(TypeClass::Pointer, 8, 8), BaseTy(baseTy) {
            Complete = BaseTy->Size != 0;
        }
    };
    ...
    public:
        ...
        ArrayType(std::shared_ptr<Type> elementType, int len)
            : Type(TypeClass::Array, len * elementType->Size, elementType->Align), ElementType(elementType), ArrayLen(len) {
    ...
    public:
        ...
        std::shared_ptr<Type> Ty;
        int Offset;
        Field(std::string_view id, std::shared_ptr<Type> ty, int offset): Id(id), Ty(ty), Offset(offset) {}
        Field(std::shared_ptr<Type> ty, int offset): Ty(ty), Offset(offset) {}
    };
